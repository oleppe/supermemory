...
class IngestionController extends Controller
{
    ...
    public function storeFiles(AddFilesRequest $request): JsonResponse
    {
        ...
        $text = $files
            ->map(fn (UploadedFile $file): string => (string) file_get_contents($file->getRealPath()))
            ->implode("\n\n");
        $filename = $files->count() === 1
            ? $files->first()->getClientOriginalName()
            : 'uploaded-files.txt';

        [$dataset, $result, $cognify] = $this->withCogneeSession($user, $this->cogneeService, function (string $token) use ($validated, $text, $filename) {
            $dataset = $this->resolveDataset($token, $validated['dataset_id'] ?? null, $validated['dataset_name'] ?? null);
